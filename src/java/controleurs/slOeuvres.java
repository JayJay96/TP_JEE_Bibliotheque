...
public class slOeuvres extends HttpServlet {
    ...
    private String supprimerOeuvre(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if(erreur.contains("FK_RESERVATION_OEUVRE"))
                erreur = "Il n'est pas possible de supprimer l'oeuvre : " + request.getAttribute("titre").toString() + " car elle a été réservée !";            
    ...
    private String connecter(HttpServletRequest request) throws Exception {
        ...
        try {
            p = new Proprietaire();
            if(p.checkLogin(request.getParameter("txtLogin"), 
                    request.getParameter("txtPwd"))){
                vueReponse = "/home.jsp";
            }
            else{
                vueReponse = "/index.jsp";    
                throw new Exception("Login ou mot de passe inconunu");
            }
            return (vueReponse);
        } catch (Exception e) {
            vueReponse = "/index.jsp"; 
